...
if "agreed" not in st.session_state:
    st.session_state.agreed = True if st.session_state.cookie.get("agreed") else False
...
if st.session_state["connected"]:
    if st.session_state.agreed:
        ...
        show_agreement()
        if st.columns(11)[1].button("I Agree"):
            st.session_state.agreed=True
            exp_date = (
                datetime.now() + timedelta(days=365)
            ).timestamp()
            st.session_state.cookie.set("agreed","True",expires_at=datetime.fromtimestamp(exp_date))
            # st.toast(":red[Something went wrong.Please try again.]")
            time.sleep(1)
            st.rerun()
